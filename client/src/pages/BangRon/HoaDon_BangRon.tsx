...
export function HoaDon_BangRon() {
  ...
  let [list, setList] = useState<BangRonBillJoinCustomer[]>([]);
  let [copyList, setCopyList] = useState<BangRonBillJoinCustomer[]>([]);
  ...
  async function loadBangRonBill() {
    try {
      await BangRonBillApi.fetchBangRonBills().then((data) => {
        if (data.length > 0) setIsEmptyList(false);
        let copyCat: BangRonBillJoinCustomer[] = [];
        data.map((item) => {
  ...
  async function updateState(
    id: string,
    input: BangRonBillApi.BangRonBillInput
  ) {
    await BangRonBillApi.updateBangRonBill(id, input);
  }
  ...
  const setState = async (
    ...
    if (stateString === "Thanh toán") {
      ...
      await RevenueApi.fetchRevenues().then((data) => {
        ...
        if (
          typeof currentTime.month === "undefined" ||
          typeof currentTime.year === "undefined"
        ) {
          currentTime.month = "0";
          currentTime.year = "0";
        }
          ...
          findItem.map((item) => {
            inputRevenue = {
              ...
              kindRevenue: {
                ...
                incomeBangRon: item.kindRevenue.incomeBangRon + +obj.totalPrice,
                ...
                bangRonOrder: item.kindRevenue.bangRonOrder + 1,
  ...
  const deleteBill = async () => {
    await BangRonBillApi.deleteBangRonBill(idDeleting);
        ...
        if (findItem.length > 0) {
          ...
          findItem.map((item) => {
            inputRevenue = {
              ...
              kindRevenue: {
                incomeDecal: item.kindRevenue.incomeDecal,
                incomeBangRon: item.kindRevenue.incomeBangRon - total,
                incomeBangHieu: item.kindRevenue.incomeBangHieu,
                incomeKhac: item.kindRevenue.incomeKhac,
                decalOrder: item.kindRevenue.decalOrder,
                bangRonOrder: item.kindRevenue.bangRonOrder - 1,
