...
export async function fetchBangRonBills(): Promise<BangRonBill[]> {
  const response = await fetch(
    "https://advertising-billboards-store-management-7jg89yi8h.vercel.app/api/bangRonBills",
    {
      method: "GET",
    }
  );
...
export async function createBangRonBill(
  bill: BangRonBillInput
): Promise<BangRonBill> {
...
export async function updateBangRonBill(
  billId: string,
  bill: BangRonBillInput
): Promise<BangRonBill> {
  const response = await fetchData(
