...
export async function fetchBangHieuBills(): Promise<BangHieuBill[]> {
  const response = await fetch(
    "https://advertising-billboards-store-management-7jg89yi8h.vercel.app/api/bangHieuBills",
    {
      method: "GET",
    }
  );
...
export async function createBangHieuBill(
  bill: BangHieuBillInput
): Promise<BangHieuBill> {
...
export async function updateBangHieuBill(
  billId: string,
  bill: BangHieuBillInput
): Promise<BangHieuBill> {
  const response = await fetchData(
